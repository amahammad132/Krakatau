...
def _convert(maker, input_, iNode):
    ...
    line = ssa_ops.Convert(
        maker.parent, arg, _charToSSAType[src_c], _charToSSAType[dest_c]
    )
...
def _field_access(maker, input_, iNode):
    ...
    line = ssa_ops.FieldAccess(
        maker.parent, iNode.instruction, (target, name, desc), args=args
    )
...
def _if_a(maker, input_, iNode):
    ...
    jump = ssa_jumps.If(
        maker.parent,
        iNode.instruction[1],
        list(map(maker.blockd.get, iNode.successors)),
        (input_.stack[-1], null),
    )
...
def _jsr(maker, input_, iNode):
    ...
    newlocals = dict(
        enumerate(
            maker.parent.makeVarFromVtype(vt, {}) for vt in retnode.out_state.locals
        )
    )
    ...
    jump = subproc.ProcCallOp(
        maker.blockd[iNode.successors[0]],
        maker.blockd[iNode.next_instruction],
        input_,
        out_slots,
    )
    return ResultDict(jump=jump, newstack=newstack)
...
def _switch(maker, input_, iNode):
    ...
    jump = ssa_jumps.Switch(
        maker.parent, maker.blockd[default], table, input_.stack[-1:]
    )
    newstack = input_.stack[:-1]
    return ResultDict(jump=jump, newstack=newstack)
