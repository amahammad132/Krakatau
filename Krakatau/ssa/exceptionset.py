...
class CatchSetManager(object):
    ...
    def new(env, chpairs):
        ...
        )  # make this ordered since OnException relies on it
    ...
    def replaceKeys(self, replace):
        self.sets = collections.OrderedDict(
            (replace.get(key, key), val) for key, val in list(self.sets.items())
        )
...
class ExceptionSet(ValueType):
    ...
    def getTopTTs(self):
        return sorted([objtypes.TypeTT(top, 0) for (top, holes) in self.pairs])
    ...
    def __sub__(self, other):
        ...
        for pair2 in other.pairs:
            ...
            pairs = itertools.chain(
                *[ExceptionSet.diffPair(subtest, pair1, pair2) for pair1 in pairs]
            )
    ...
    def __str__(self):  # pragma: no cover
        parts = [
            ("{} - [{}]".format(top, ", ".join(sorted(holes))) if holes else top)
            for top, holes in self.pairs
        ]
    ...
    def diffPair(
        subtest, pair1, pair2
    ):  # subtract pair2 from pair1. Returns a list of new pairs
        ...
        if subtest(t1, t2):  # t2 >= t1
            if any(subtest(t1, h) for h in holes2):
                ...
                holes2 = [
                    h
                    for h in holes2
                    if subtest(h, t1) and not any(subtest(h, h2) for h2 in holes1)
                ]
            ...
            if any(subtest(t2, h) for h in holes1):
                ...
                newpairs = [
                    (t1, ExceptionSet.reduceHoles(subtest, list(holes1) + [t2]))
                ]
    ...
    def reduce(env, pairs):
        ...
        pairs = [
            pair for pair in pairs if pair[0] not in pair[1]
        ]  # remove all degenerate pairs
        ...
        while pairs:
            ...
            for epair in newpairs[:]:
                ...
                if subtest(top, etop) and (
                    top in eholes or not any(subtest(top, ehole) for ehole in eholes)
                ):
                    new = ExceptionSet.mergePair(subtest, epair, pair)
                    newpairs, pairs = [new], [
                        p for p in newpairs if p is not epair
                    ] + pairs
                    break
                # existing pair can be merged into new pair
                elif subtest(etop, top) and (
                    etop in holes or not any(subtest(etop, hole) for hole in holes)
                ):
