...
class IUshr(BaseOp):
    ...
    def propagateConstraints(self, x, y):
        ...
        if y.min < y.max:
            intmax = (M // 2) - 1
            return return_(
                IntConstraint.range(x.width, min(x.min, 0), max(x.max, intmax))
            )
        shift = y.min % x.width
        if not shift:
            return return_(x)
...
class IDiv(BaseOp):
    def __init__(self, parent, args):
        ...
        self.outExceptionCons = ObjectConstraint.fromTops(
            parent.env, [], exec_tts, nonnull=True
        )
    ...
    def propagateConstraints(self, x, y):
        ...
        for xv, yv in itertools.product(xvals, yvals):
            if xv == intmin and yv == -1:
                ...
                xv * yv < 0
            ):  # Unlike Python, Java rounds to 0 so opposite sign case must be handled specially
                vals.add(-(-xv // yv))
            else:
                vals.add(xv // yv)
