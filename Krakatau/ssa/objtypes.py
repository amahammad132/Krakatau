...
def withDimInc(tt, inc):
    return TypeTT(baset(tt), dim(tt) + inc)


def withNoDim(tt):
    return TypeTT(baset(tt), 0)


def isBaseTClass(tt):
    return not baset(tt).startswith(".")


def className(tt):
    return baset(tt) if not baset(tt).startswith(".") else None


def primName(tt):
    return baset(tt)[1:] if baset(tt).startswith(".") else None
...
def isSubtype(env, x, y):
    ...
    if ydim > xdim:
        ...
        xdim > ydim
    ):  # TODO - these constants should be defined in one place to reduce risk of typos
        return yname in (
            "java/lang/Object",
            "java/lang/Cloneable",
...
def commonSupertype(env, tts):
    assert hasattr(
        env, "getClass"
    )  # catch common errors where we forget the env argument
...
_verifierConvert = {
    vtypes.T_INT: IntTT,
    vtypes.T_FLOAT: FloatTT,
    vtypes.T_LONG: LongTT,
    vtypes.T_DOUBLE: DoubleTT,
    vtypes.T_SHORT: ShortTT,
    vtypes.T_CHAR: CharTT,
    vtypes.T_BYTE: ByteTT,
    vtypes.T_BOOL: BoolTT,
    vtypes.T_NULL: NullTT,
