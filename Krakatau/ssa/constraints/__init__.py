...
def fromConstant(env, var):
    ...
    if ssa_type[0] == SSA_INT[0]:
        ...
        return ObjectConstraint.fromTops(
            env, *objtypes.declTypeToActual(env, var.decltype)
        )
    ...
    try:
        ...
        if var.decltype is not None:
            if var.decltype == objtypes.NullTT:
                return ObjectConstraint.constNull(env)
            return ObjectConstraint.fromTops(
                env, *objtypes.declTypeToActual(env, var.decltype), nonnull=isnew
            )
        else:
            return ObjectConstraint.fromTops(
                env, [objtypes.ObjectTT], [], nonnull=isnew
            )
...
def returnOrThrow(rval, eval):
    return OpReturnInfo(rval, eval, False)


def maybeThrow(eval):
    return OpReturnInfo(None, eval, False)


def throw(eval):
    return OpReturnInfo(None, eval, True)


def return_(rval):
    return OpReturnInfo(rval, None, False)
