...
class TypeConstraint(ValueType):
    ...
    def _key(self):
        return self.supers, self.exact

    def __bool__(self):
        return bool(self.supers or self.exact)
    ...
    def isBoolOrByteArray(self):
        ...
        return objtypes.dim(tt1) == objtypes.dim(tt2) and sorted(bases) == [
            objtypes.baset(objtypes.BoolTT),
            objtypes.baset(objtypes.ByteTT),
        ]
    ...
    def join(*cons):
        ...
        supers_l, exact_l = list(zip(*(c._key() for c in cons)))
        ...
        for c in cons:
            newexact = [
                x for x in newexact if x in c.exact or isAnySubtype(env, x, c.supers)
            ]
...
class ObjectConstraint(ValueType):
    ...
    def _key(self):
        return self.null, self.types

    def isConstNull(self):
        return self.null and not self.types
    ...
    def __str__(self):  # pragma: no cover
        if not self.types:
            return "Obj(null)"
        return "Obj({}, {}, {})".format(
            self.null, sorted(self.types.supers), sorted(self.types.exact)
        )
