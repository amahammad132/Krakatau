...
class BaseJump(SSAFunctionBase):
    ...
    def getSuccessors(self):
        return self.getNormalSuccessors() + self.getExceptSuccessors()

    def getSuccessorPairs(self):
        return [(x, False) for x in self.getNormalSuccessors()] + [
            (x, True) for x in self.getExceptSuccessors()
        ]
    ...
    def clone(self):
        return copy.copy(self)  # overriden by classes which need to do a deep copy
