...
class VerifierTypesState(object):
    ...
    def copy(self):
        return VerifierTypesState(self.stack, self.locals, self.masks)

    def withExcept(self, t):
        return VerifierTypesState([t], self.locals, self.masks)
    ...
    def returnTo(self, called, jsrstate):
        ...
        zipped = itertools.zip_longest(
            self.locals, jsrstate.locals, fillvalue=T_INVALID
        )
    ...
    def merge(self, other, env):
        ...
        self.stack = [
            mergeTypes(env, new, old) for old, new in zip(self.stack, other.stack)
        ]
        self.locals = [
            mergeTypes(env, new, old) for old, new in zip(self.locals, other.locals)
        ]
...
def stateFromInitialArgs(args):
    return VerifierTypesState([], args[:], [])
...
class InstructionNode(object):
    ...
    def __init__(self, code, offsetToIndex, indexToOffset, key):
        ...
        )  # if it is this will cause problems with origin tracking
    ...
    def _removeInterface(self, vt):
        if (
            vt.tag == ".obj"
            and vt.extra is not None
            and self.env.isInterface(vt.extra, forceCheck=True)
        ):
    ...
    def update(self, iNodes, exceptions):
        ...
        if successors is None:
            ...
            temp = [
                n.next_instruction
                for n in list(iNodes.values())
                if (n.op == ops.JSR and n.instruction[1] == called)
            ]
        ...
        for (start, end, handler, except_info) in exceptions:
            if start <= self.key < end:
                self._mergeSingleSuccessor(
                    handler, self.state.withExcept(except_info), iNodes, True
                )
                if (
                    self.op == ops.INVOKEINIT
                ):  # two cases since the ctor may suceed or fail before throwing
                    self._mergeSingleSuccessor(
                        handler, newstate.withExcept(except_info), iNodes, True
                    )
    ...
    def __str__(self):  # pragma: no cover
        ...
        if self.visited:
            ...
            if self.state.masks:
                lines.append("Masks:")
                lines += [
                    "\t{}: {}".format(entry, sorted(cset))
                    for entry, cset in self.state.masks
                ]
        else:
...
def verifyBytecode(code):
    ...
    args, rval = parseUnboundMethodDescriptor(
        method.descriptor, class_.name, method.static
    )
    env = class_.env
    ...
    for raw in code.except_raw:
        if not (
            (0 <= raw.start < raw.end)
            and (raw.start in keys)
            and (raw.handler in keys)
            and (raw.end in keys or raw.end == code.codelen)
        ):

            keylist = sorted(keys) + [code.codelen]
            msg = "Illegal exception handler: {}\nValid offsets are: {}".format(
                raw, ", ".join(map(str, keylist))
            )
