...
valid_tags = [
    "." + _x
    for _x in "int float double long obj new init address byte short char boolean".split()
]
...
T_BOOL = _makeinfo(
    ".boolean"
)  # Hotspot doesn't have a bool type, but we can use this elsewhere
...
def mergeTypes(env, t1, t2):
    ...
    if t1.dim or t2.dim:
        ...
        if t1.dim == t2.dim:
            ...
            return (
                t1
                if withNoDimension(t1) in (CLONE_INFO, SERIAL_INFO)
                else T_ARRAY(OBJECT_INFO, t1.dim)
            )
