    ...
    while desc_str:
        ...
        if desc_str[0] == "L":
            ...
            if desc_str[0] not in baseTypes:
                raise ValueError(
                    "Unrecognized code {} in descriptor".format(desc_str[0])
                )
...
def parseFieldDescriptor(desc_str, unsynthesize=True):
    ...
    if len(rval) != cat:
        raise ValueError(
            "Incorrect number of fields in descriptor, expected {} but found {}".format(
                cat, len(rval)
            )
        )
    return rval
...
def parseMethodDescriptor(desc_str, unsynthesize=True):
    ...
    if lp_pos < 0 or desc_str[lp_pos] != ")":
        raise ValueError(
            "Unable to split method descriptor into arguments and return type"
        )
