...
newarr_codes = dict(
    list(
        zip("boolean char float double byte short int long".split(), list(range(4, 12)))
    )
)
...
et_rtags = dict(
    list(
        zip(
            list(map(ord, "BCDFIJSZsec@[")),
            "byte char double float int long short boolean string enum class annotation array".split(),
        )
    )
)
