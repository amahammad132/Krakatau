...
class Writer(object):
    ...
    def u16(self, x):
        self.b.extend(struct.pack(">H", x))

    def s16(self, x):
        self.b.extend(struct.pack(">h", x))

    def u32(self, x):
        self.b.extend(struct.pack(">I", x))

    def s32(self, x):
        self.b.extend(struct.pack(">i", x))

    def u64(self, x):
        self.b.extend(struct.pack(">Q", x))
        ...
        for pos, lbl, base, dtype in self.lblphs:
            ...
            if dtype == "s16":
                if not -1 << 15 <= offset < 1 << 15:
                    error(
                        "Label offset must fit in signed 16 bit int. (offset is {})".format(
                            offset
                        ),
                        tok,
                    )
                ...
                if not 0 <= offset < 1 << 16:
                    error(
                        "Label offset must fit in unsigned 16 bit int. (offset is {})".format(
                            offset
                        ),
                        tok,
                    )
                ...
                if not -1 << 31 <= offset < 1 << 31:
                    error(
                        "Label offset must fit in signed 32 bit int. (offset is {})".format(
                            offset
                        ),
                        tok,
                    )
    ...
    def __iadd__(self, other):
        ...
        assert len(other.lblphs) == len(other._ph8s) + len(other._ph16s) + len(
            other._ph32s
        )
        ...
        self.lblphs.extend(
            (pos + offset, lbl, base, dtype) for pos, lbl, base, dtype in other.lblphs
        )
