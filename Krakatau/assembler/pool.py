...
class Ref(object):
    def __init__(
        self, tok, index=None, symbol=None, type=None, refs=None, data=None, isbs=False
    ):
        self.tok = tok
        self.isbs = isbs
        self.index = index
    ...
    def _deepdata(self, pool, error, defstack=()):
        ...
        if (
            len(defstack) > 5
        ):  # Maximum legitimate depth is 5: ID -> BS -> MH -> F -> NAT -> UTF
            error_args = [
                "Constant pool definitions cannot be nested more than 5 deep (excluding raw references).",
                self.tok,
            ]
        ...
        return (
            self.type,
            self.data,
            tuple(ref._deepdata(pool, error, defstack + (self,)) for ref in self.refs),
        )
...
class PoolSub(object):
    ...
    def adddef(self, lhs, rhs, error):
        ...
        if lhs.israw():
            ...
            if lhs.index in self.slots:
                error(
                    "Conflicting raw reference definition",
                    lhs.tok,
                    "Conflicts with previous definition:",
                    self.slot_def_tokens[lhs.index],
                )
            ...
            if rhs.type in ("Long", "Double"):
                if lhs.index + 1 in self.slots:
                    error(
                        ...
                        lhs.tok,
                        "Conflicts with previous definition:",
                        self.slot_def_tokens[lhs.index + 1],
                    )
            ...
            if lhs.symbol in self.symdefs:
                error(
                    "Duplicate symbolic reference definition",
                    lhs.tok,
                    "Previously defined here:",
                    self.symdefs[lhs.symbol][0],
                )
            self.symdefs[lhs.symbol] = lhs.tok, rhs

    def freezedefs(self, pool, error):
        self.defsfrozen = True
    ...
    def write(self, pool, error):
        ...
        self.dirtyslotdefs = (
            None  # make sure we don't accidently add entries after size is taken
        )
        ...
        for i in range(size):
            ...
            if self.isbs:
                ...
                if len(w) >= (1 << 32):
                    error(
                        "Maximum BootstrapMethods length is {} bytes.".format(
                            (1 << 32) - 1
                        ),
                        v.tok,
                    )
...
class Pool(object):
    ...
    def sub(self, ref):
        return self.bs if ref.isbs else self.cp
