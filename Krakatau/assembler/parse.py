...
class Parser(object):
    ...
    def ateol(self):
        return self.hastype("NEWLINES")

    def atendtok(self):
        return self.hasv(".end")
    ...
    def u8(a):
        return a.boundedint(0, 1 << 8)

    def u16(a):
        return a.boundedint(0, 1 << 16)

    def u32(a):
        return a.boundedint(0, 1 << 32)

    def s8(a):
        return a.boundedint(-1 << 7, 1 << 7)

    def s16(a):
        return a.boundedint(-1 << 15, 1 << 15)

    def s32(a):
        return a.boundedint(-1 << 31, 1 << 31)
    ...
    def string(a, maxlen=65535):
        ...
        if len(val) > maxlen:
            a.error(
                "Maximum string length here is {} bytes ({} found).".format(
                    maxlen, len(val)
                ),
                tok,
            )
    ...
    def word(a, maxlen=65535):
        ...
        if len(val) > maxlen:
            a.error(
                "Maximum identifier length is {} bytes ({} found).".format(
                    maxlen, len(val)
                ),
                tok,
            )
    ...
    def fmimref(a, typeguess):
        ...
        if a.hastype("REF"):
            ...
            if a.hastype("WORD"):
                return pool.Ref(
                    first.tok, type=typeguess, refs=[first, a.natref()]
                )  # Krakatau v0
            return first
        ...
        if 1 <= len(words) <= 2 and a.hastype("REF"):  # Krakatau v0
            ...
            if len(words) == 2:
                ...
                return pool.Ref(
                    cls.tok, type=typeguess, refs=[cls, pool.nat(name, a.utfref())]
                )
    ...
    def mhnotref(a, tok):
        if a.hasany(codes.handle_codes):
            ...
            return pool.Ref(
                tok, type="MethodHandle", data=code, refs=[a.ref_or_tagged_const()]
            )
        a.fail()
    ...
    def ref_or_tagged_const(a, isbs=False, methodhandle=False, invokedynamic=False):
        ...
        if isbs and not ref.isbs:
            a.error(
                "Expected bootstrap reference, found constant pool reference.", ref.tok
            )
        elif not isbs and ref.isbs:
            a.error(
                "Expected constant pool reference, found bootstrap reference.", ref.tok
            )
        return ref
    ...
    def class_item(a):
        a.try_const_def() or a.try_field() or a.try_method() or a.try_attribute(
            a.cls, allowed=("Record")
        ) or a.fail()
    ...
    def try_const_def(a):
        if a.hasany([".const", ".bootstrap"]):
            ...
            if lhs.israw() and (rhs.israw() or rhs.issym()):
                a.error(
                    "Raw references cannot be aliased to another reference.", rhs.tok
                )
            a.eol()
    ...
    def try_instruction(a):
        ...
        if a.hasany(OP_NONE):
            ...
            w.u8(OPNAME_TO_BYTE[a.consume().val]), w.ref(
                a.fmimref(OP_FMIM_TO_GUESS[op])
            )
            ...
            if a.hastype("INT_LITERAL"):
                ...
                if ref.israw() or ref.issym():
                    a.error(
                        "Method descriptor must be specified inline when argument count is omitted.",
                        ref.tok,
                    )
                ref = ref.refs[1]  # NAT
                if ref.israw() or ref.issym():
            ...
            w.u8(OPNAME_TO_BYTE[a.consume().val]), w.ref(
                a.ref_or_tagged_const(invokedynamic=True)
            ), w.u16(0)
            ...
            while not a.hasv("default"):
                ...
                if low + len(jumps) - 1 > (1 << 31) - 1:
                    a.error(
                        "Table switch index must be at most 2147483647.", jumps[-1].tok
                    )
            if not jumps:
            ...
            while not a.hasv("default"):
                ...
                if key in jumps:
                    a.error(
                        "Duplicate lookupswitch key.",
                        keytok,
                        "Key previously defined here:",
                        prevtoks[key],
                    )
        ...
        if len(w) > a.code.maxcodelen:
            self.error(
                "Maximum bytecode length is {} (current {}).".format(
                    a.code.maxcodelen, len(w)
                ),
                starttok,
            )
        return True
    ...
    def _check_delta(a, tok, frame_type, delta_offset, maxv):
        if delta_offset > maxv:
            a.error(
                'Stack frame type "{}" must appear at most {} bytes after the previous frame (actual offset is {}).'.format(
                    frame_type, maxv + 1, delta_offset + 1
                ),
                tok,
            )
    ...
    def maybe_named_attribute(a, wrapper_attr, allowed):
        ...
        if a.tryv(".annotationdefault"):
            ...
            a.eol(), a.list(w, a.atendtok, a._innerclasses_item), a.val(".end"), a.val(
                "innerclasses"
            )
        elif a.tryv(".linenumbertable"):
            attr, w = create(b"LineNumberTable")
            a.eol(), a.list(w, a.atendtok, a._linenumber_item), a.val(".end"), a.val(
                "linenumbertable"
            )
        elif a.tryv(".localvariabletable"):
            attr, w = create(b"LocalVariableTable")
            a.eol(), a.list(w, a.atendtok, a._localvariabletable_item), a.val(
                ".end"
            ), a.val("localvariabletable")
        elif a.tryv(".localvariabletypetable"):
            attr, w = create(
                b"LocalVariableTypeTable"
            )  # reuse _localvariabletable_item func
            a.eol(), a.list(w, a.atendtok, a._localvariabletable_item), a.val(
                ".end"
            ), a.val("localvariabletypetable")
        elif a.tryv(".methodparameters"):
            attr, w = create(b"MethodParameters")
            a.eol(), a.listu8(w, a.atendtok, a._methodparams_item), a.val(
                ".end"
            ), a.val("methodparameters")
        elif a.tryv(".module"):
            print(
                "Warning! Assembler syntax for Java 9 modules is experimental and subject to change. Please file an issue on Github if you have any opinions or feedback about the syntax"
            )
            attr, w = create(b"Module")
            w.ref(a.utfref()), w.u16(a.flags()), a.consume(), w.ref(
                a.utfref()
            ), a.eol(),
            ...
            a.eol(), a.list(w, a.atendtok, a._record_item), a.val(".end"), a.val(
                "record"
            )
    ...
    def _package_item(a, w):
        w.ref(a.clsref(tag="Package"))

    def _module_item(a, w):
        w.ref(a.clsref(tag="Package"))

    def _innerclasses_item(a, w):
        w.ref(a.clsref()), w.ref(a.clsref()), w.ref(a.utfref()), w.u16(
            a.flags()
        ), a.eol()

    def _linenumber_item(a, w):
        w.lbl(a.lbl(), 0, "u16"), w.u16(a.u16()), a.eol()

    def _methodparams_item(a, w):
        w.ref(a.utfref()), w.u16(a.flags()), a.eol()

    def _localvariabletable_item(a, w):
        ind, _, name, desc, _range, _ = (
            a.u16(),
            a.val("is"),
            a.utfref(),
            a.utfref(),
    ...
    def _mod_list(a, w, startdir, cb):
        a.list(w, lambda: not a.tryv(startdir), cb)

    def _mod_requires_item(a, w):
        w.ref(a.clsref(tag="Module")), w.u16(a.flags()), a.val("version"), w.ref(
            a.utfref()
        ), a.eol()
    ...
    def _mod_uses_item(a, w):
        w.ref(a.clsref()), a.eol()
    ...
    def annotation_line(a, w):
        a.val(".annotation"), a.annotation_contents(w), a.val(".end"), a.val(
            "annotation"
        ), a.eol()
    ...
    def element_value(a, w):
        thunk(a._element_value(w))

    def annotation_contents(a, w):
