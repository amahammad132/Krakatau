...
class ReferencePrinter(object):
    def __init__(self, clsdata, roundtrip):
        ...
        for attr in clsdata.getattrs(b"InnerClasses"):
            ...
            for _ in range(r.u16()):
                ...
                if inner != outer and clsdata.pool.getclsutf(
                    inner
                ) == clsdata.pool.getclsutf(outer):
    ...
    def _int(self, x):
        return str(make_signed(x, 32))

    def _long(self, x):
        return str(make_signed(x, 64)) + "L"

    def _float(self, x):
        return self._float_or_double(x, 23, 8, "f", "<0x{:08X}>")

    def _double(self, x):
        return self._float_or_double(x, 52, 11, "", "<0x{:016X}>")
    ...
    def fmimref(self, ind):
        ...
        if self.cpslots[ind].tag in ["Field", "Method", "InterfaceMethod"]:
            ind2, ind3 = self.cpslots[ind].refs
            return " ".join(
                [self.cpslots[ind].tag, self.clsref(ind2), self.natref(ind3)]
            )
        return self.symref(ind)
    ...
    def mhnotref(self, ind):
        ...
        return (
            codes.handle_rcodes[slot.data]
            + " "
            + self.taggedref(
                slot.refs[0], allowed=["Field", "Method", "InterfaceMethod"]
            )
        )
    ...
    def ldcrhs(self, ind):
        ...
        if t == "Int":
            ...
            if temp is not None:
                return temp
            return self.symref(ind)
        return self.taggedref(
            ind, allowed=["Class", "MethodHandle", "MethodType", "Dynamic"]
        )
...
class Disassembler(object):
    ...
    ###########################################################################
    def extrablankline(a):
        a.eol()

    def ref(a, ind, isbs=False):
        a.val(a.refprinter.ref(ind, isbs))

    def utfref(a, ind):
        a.val(a.refprinter.utfref(ind))

    def clsref(a, ind, tag="Class"):
        a.val(a.refprinter.clsref(ind, tag))

    def natref(a, ind):
        a.val(a.refprinter.natref(ind))

    def fmimref(a, ind):
        a.val(a.refprinter.fmimref(ind))

    def taggedbs(a, ind):
        a.val(a.refprinter.bsnotref(ind, tagged=True))

    def taggedconst(a, ind):
        a.val(a.refprinter.taggedconst(ind))

    def taggedref(a, ind):
        a.val(a.refprinter.taggedref(ind))

    def ldcrhs(a, ind):
        a.val(a.refprinter.ldcrhs(ind))
    ...
    def disassemble(a):
        ...
        a.val(".class"), a.flags(cls.access, flags.RFLAGS_CLASS), a.clsref(
            cls.this
        ), a.eol()
    ...
    def field(a, f):
        a.val(".field"), a.flags(f.access, flags.RFLAGS_FIELD), a.utfref(
            f.name
        ), a.utfref(f.desc)
    ...
    def method(a, m):
        a.extrablankline()
        a.val(".method"), a.flags(m.access, flags.RFLAGS_METHOD), a.utfref(
            m.name
        ), a.val(":"), a.utfref(m.desc), a.eol()
        a.indentlevel += 1
    ...
    def code(a, r):
        ...
        a.val(".code"), a.val("stack"), a.int(c.stack), a.val("locals"), a.int(
            c.locals
        ), a.eol()
        a.indentlevel += 1
    ...
    def attribute_fallible(a, name, attr):
        ...
        try:
            ...
            if name == b"AnnotationDefault":
                ...
                a.indented_line_list(
                    r, a._localvariabletable_item, "localvariabletable"
                )
            elif name == b"LocalVariableTypeTable":
                a.indented_line_list(
                    r, a._localvariabletable_item, "localvariabletypetable"
                )
            elif name == b"MethodParameters":
                a.indented_line_list(
                    r, a._methodparams_item, "methodparameters", bytelen=True
                )
            elif name == b"Module":
                a.module_attr(r)
                ...
                if b"Type" in name:
                    ...
                    a.indented_line_list(
                        r, a.param_annotation_line, "runtime", False, bytelen=True
                    )
                else:
    ...
    def module_attr(a, r):
        a.val(".module"), a.clsref(r.u16(), tag="Module"), a.flags(
            r.u16(), flags.RFLAGS_MOD_OTHER
        )
        a.val("version"), a.utfref(r.u16()), a.eol()
        a.indentlevel += 1

        for _ in range(r.u16()):
            a.sol(), a.val(".requires"), a.clsref(r.u16(), tag="Module"), a.flags(
                r.u16(), flags.RFLAGS_MOD_REQUIRES
            ), a.val("version"), a.utfref(r.u16()), a.eol()

        for dir_ in (".exports", ".opens"):
            for _ in range(r.u16()):
                a.sol(), a.val(dir_), a.clsref(r.u16(), tag="Package"), a.flags(
                    r.u16(), flags.RFLAGS_MOD_OTHER
                )
                count = r.u16()
                if count:
    ...
    def _innerclasses_item(a, r):
        a.clsref(r.u16()), a.clsref(r.u16()), a.utfref(r.u16()), a.flags(
            r.u16(), flags.RFLAGS_CLASS
        )

    def _linenumber_item(a, r):
        a.try_lbl(r.u16()), a.int(r.u16())
    ...
    def _methodparams_item(a, r):
        a.utfref(r.u16()), a.flags(r.u16(), flags.RFLAGS_MOD_OTHER)
    ...
    def annotation_line(a, r):
        a.val(".annotation"), a.annotation_contents(r), a.sol(), a.val(".end"), a.val(
            "annotation"
        )
    ...
    def ta_target_path(a, r):
        a.sol(), a.indented_line_list(
            r, a._type_path_segment, "typepath", bytelen=True
        ), a.eol()
    ...
    def element_value(a, r):
        thunk(a._element_value(r))

    def annotation_contents(a, r):
        thunk(a._annotation_contents(r))
    ...
    def _element_value(a, r):
        ...
        a.val(tag)
        if tag == "annotation":
            (yield a._annotation_contents(r)), a.sol(), a.val(".end"), a.val(
                "annotation"
            )
        elif tag == "array":
            a.eol()
