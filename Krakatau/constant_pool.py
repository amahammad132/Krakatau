...
cpoolInfo_t = collections.namedtuple("cpoolInfo_t", ["name", "tag", "recoverArgs"])

Utf8 = cpoolInfo_t("Utf8", 1, (lambda self, s: (s,)))

Class = cpoolInfo_t("Class", 7, (lambda self, n_id: self.getArgs(n_id)))

NameAndType = cpoolInfo_t(
    "NameAndType", 12, (lambda self, n, d: self.getArgs(n) + self.getArgs(d))
)

Field = cpoolInfo_t(
    "Field", 9, (lambda self, c_id, nat_id: self.getArgs(c_id) + self.getArgs(nat_id))
)

Method = cpoolInfo_t(
    "Method", 10, (lambda self, c_id, nat_id: self.getArgs(c_id) + self.getArgs(nat_id))
)

InterfaceMethod = cpoolInfo_t(
    "InterfaceMethod",
    11,
    (lambda self, c_id, nat_id: self.getArgs(c_id) + self.getArgs(nat_id)),
)

String = cpoolInfo_t("String", 8, (lambda self, n_id: self.getArgs(n_id)))

Int = cpoolInfo_t("Int", 3, (lambda self, s: (s,)))

Long = cpoolInfo_t("Long", 5, (lambda self, s: (s,)))

Float = cpoolInfo_t("Float", 4, (lambda self, s: decodeFloat(s)))

Double = cpoolInfo_t("Double", 6, (lambda self, s: decodeDouble(s)))

MethodHandle = cpoolInfo_t(
    "MethodHandle", 15, (lambda self, t, n_id: (t,) + self.getArgs(n_id))
)

MethodType = cpoolInfo_t("MethodType", 16, (lambda self, n_id: self.getArgs(n_id)))

InvokeDynamic = cpoolInfo_t(
    "InvokeDynamic", 18, (lambda self, bs_id, nat_id: (bs_id,) + self.getArgs(nat_id))
)
