...
def loadConstValue(cpool, index):
    ...
    tt = {
        "Int": objtypes.IntTT,
        "Long": objtypes.LongTT,
        "Float": objtypes.FloatTT,
        "Double": objtypes.DoubleTT,
        "String": objtypes.StringTT,
    }[entry_type]
...
def _getField(field):
    ...
    dtype = objtypes.verifierToSynthetic(
        parseFieldDescriptor(desc, unsynthesize=False)[0]
    )
    ...
    if field.static:
        ...
        const_attrs = [
            data for name, data in field.attributes if name == "ConstantValue"
        ]
        if const_attrs:
    ...
    return ast2.FieldDef(
        " ".join(flags), ast.TypeName(dtype), field.class_, field.name, desc, initexpr
    )
...
def generateAST(cls, cb, skip_errors, method=None, add_throws=False):
    ...
    myflags = [
        x.lower()
        for x in sorted(cls.flags)
        if x not in ("INTERFACE", "SUPER", "SYNTHETIC", "ANNOTATION", "ENUM")
    ]
    isInterface = "INTERFACE" in cls.flags

    superc = cls.supername
    interfaces = [
        cls.cpool.getArgsCheck("Class", index) for index in cls.interfaces_raw
    ]  # todo - change when class actually loads interfaces

    field_defs = [_getField(f) for f in cls.fields]
    method_defs = [
        _getMethod(m, cb, forbidden_identifiers, skip_errors) for m in methods
    ]
    if add_throws:
        throws.addSingle(cls.env, method_defs)
    return ast2.ClassDef(
        " ".join(myflags),
        isInterface,
        cls.name,
        superc,
