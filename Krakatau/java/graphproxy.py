...
class BlockProxy(object):
    def __init__(self, key, counter, block=None):
        ...
        self.eassigns = (
            {}
        )  # exception edge assignments, used after try constraint creation
    ...
    def replaceSuccessors(self, rmap):
        ...
        if self.block is not None:
            d1 = self.blockdict
            self.blockdict = {
                (b.key, t): update(d1[b.key, t])
                for (b, t) in self.block.jump.getSuccessorPairs()
            }
...
def createGraphProxy(ssagraph):
    ...
    for n in nodes:
        ...
        if n.bkey == ssagraph.entryKey:
            ...
            invars = (
                ssagraph.inputArgs
            )  # store them in the node so we don't have to keep track seperately
            invars = [
    ...
    for n in nodes:
        assert (
            len(intypes[n.bkey]) != 2
        )  # should have been handled by graph.splitDualInedges()
