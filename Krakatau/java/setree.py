...
class SEBlockItem(object):
    def __init__(self, node):
        self.successors = (
            node.norm_suc_nl
        )  # don't include backedges or exceptional edges
        self.node = node
...
class SESwitch(object):
    def __init__(self, head, newscopes):
        ...
        keysets = {
            head.node.blockdict[b.key, False]: jump.reverse.get(b)
            for b in jump.getNormalSuccessors()
        }
        assert list(keysets.values()).count(None) == 1
