...
class DUBlock(object):
    def __init__(self, key):
        ...
        self.lines = (
            []
        )  # 3 types of lines: ('use', var), ('def', (var, var2_opt)), or ('canthrow', None)
        self.e_successors = []
        self.n_successors = []
        self.vars = None  # vars used or defined within the block. Does NOT include caught exceptions

    def canThrow(self):
        return ("canthrow", None) in self.lines
...
class DUGraph(object):
    ...
    def visitScope(
        ...
        catch_stack,
        caught_except=None,
        myexcept_parents=None,
        head_block=None,
    ):
        # catch_stack is copy on modify
        if head_block is None:
            head_block = block = self.makeBlock(
                scope.continueKey, break_dict, caught_except, myexcept_parents
            )
        else:
            block = head_block
        ...
        for stmt in scope.statements:
            if isinstance(
                stmt, (ast.ExpressionStatement, ast.ThrowStatement, ast.ReturnStatement)
            ):
            ...
            if isinstance(stmt, (ast.IfStatement, ast.SwitchStatement)):
                ...
                body_block = self.visitScope(
                    stmt.getScopes()[0], break_dict, catch_stack
                )
                ...
                for cdecl, catchb in stmt.pairs:
                    ...
                    self.visitScope(
                        catchb, break_dict, catch_stack, cdecl.local, parents
                    )
            ...
            if not isinstance(
                stmt, ast.StatementBlock
            ):  # if we passed it to subscope, it will be finished in the subcall
                self.finishBlock(block, catch_stack)

            if (
                stmt.breakKey is not None
            ):  # start new block after return from compound statement
                block = self.makeBlock(stmt.breakKey, break_dict, None, None)
            else:
                block = (
                    None  # should never be accessed anyway if we're exiting abruptly
                )
    ...
    def makeCFG(self, root):
        ...
        self.entry = self.blocks[
            0
        ]  # entry point should always be first block generated

        reached = graph_util.topologicalSort(
            [self.entry], lambda block: (block.n_successors + block.e_successors)
        )
