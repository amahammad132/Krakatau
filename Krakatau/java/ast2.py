...
class MethodDef(object):
    ...
    def print_(self, printer, print_):
        ...
        if self.isStaticInit:
            ...
            header += "{}{} {}({})".format(
                self.flagstr, print_(self.retType), escape(name), argstr
            )
    ...
    def tree(self, printer, tree):
        return {
            ...
            "params": list(map(tree, self.paramDecls)),
...
class FieldDef(object):
    ...
    def print_(self, printer, print_):
        ...
        if self.expr is not None:
            return "{}{} {} = {};".format(
                self.flagstr, print_(self.type_), name, print_(self.expr)
            )
...
class ClassDef(object):
    def __init__(self, flags, isInterface, name, superc, interfaces, fields, methods):
        ...
        self.super = (
            ast.TypeName(objtypes.TypeTT(superc, 0)) if superc is not None else None
        )
        self.interfaces = [
            ast.TypeName(objtypes.TypeTT(iname, 0)) for iname in interfaces
        ]
        self.fields = fields
    ...
    def tree(self, printer, tree):
        return {
            ...
            "interfaces": list(map(tree, self.interfaces)),
            "fields": list(map(tree, self.fields)),
            "methods": list(map(tree, self.methods)),
        }
