...
class VariableDeclarator(object):
    ...
    def tree(self, printer, tree):
        return [tree(self.typename), tree(self.local)]
...
class ExpressionStatement(JavaStatement):
    ...
    def print_(self, printer, print_):
        return print_(self.expr) + ";"

    def tree(self, printer, tree):
        return [self.__class__.__name__, tree(self.expr)]
...
class LocalDeclarationStatement(JavaStatement):
    ...
    def tree(self, printer, tree):
        return [self.__class__.__name__, tree(self.expr), tree(self.decl)]
...
class ReturnStatement(JavaStatement):
    ...
    def print_(self, printer, print_):
        return (
            "return {};".format(print_(self.expr))
            if self.expr is not None
            else "return;"
        )
...
class ThrowStatement(JavaStatement):
    ...
    def print_(self, printer, print_):
        return "throw {};".format(print_(self.expr))
...
class JumpStatement(JavaStatement):
    ...
    def tree(self, printer, tree):
        return [self.__class__.__name__, self.keyword, self.label]
...
class TryStatement(LazyLabelBase):
    ...
    def getScopes(self):
        return (self.tryb,) + list(zip(*self.pairs))[1]
...
class SwitchStatement(LazyLabelBase):
    ...
    def getScopes(self):
        return list(zip(*self.pairs))[1]

    def hasDefault(self):
        return None in list(zip(*self.pairs))[0]
    ...
    def print_(self, printer, print_):
        ...
        if isinstance(self.expr, BinaryInfix) and len(body.statements) > 1:
            ...
            if isinstance(last_statement, ExpressionStatement):
                ...
                last_vars = {
                    expr for expr in last.postFlatIter() if isinstance(expr, Local)
                }
                ...
                if last.isLocalAssign() and all(
                    v in self.expr.params for v in last_vars
                ):
...
class StatementBlock(LazyLabelBase):
    ...
    def doesFallthrough(self):
        return self.jumpKey is None or self.jumpKey == self.breakKey
    ...
    def join(*scopes):
        blists = [
            s.bases for s in scopes if s is not None
        ]  # allow None to represent the universe (top element)
        if not blists:
            return None
    ...
    def tree(self, printer, tree):
        return ["BlockStatement", self.label, list(map(tree, self.statements))]
...
def isReferenceType(tt):
    return (
        tt == objtypes.NullTT
        or objtypes.dim(tt)
        or (objtypes.className(tt) is not None)
    )
...
class JavaExpression(object):
    ...
    def complexity(self):
        return 1 + max(e.complexity() for e in self.params) if self.params else 0
    ...
    def tree(self, printer, tree):
        return [self.__class__.__name__, list(map(tree, self.params))]
    ...
    def isLocalAssign(self):
        return isinstance(self, Assignment) and isinstance(self.params[0], Local)

    def __repr__(self):  # pragma: no cover
        return (
            type(self).__name__.rpartition(".")[-1]
            + " "
            + visitor.DefaultVisitor().visit(self)
        )
...
class ArrayAccess(JavaExpression):
    ...
    @property
    def dtype(self):
        return objtypes.withDimInc(self.params[0].dtype, -1)
...
class ArrayCreation(JavaExpression):
    ...
    def tree(self, printer, tree):
        return [self.__class__.__name__, list(map(tree, self.params)), self.dim]
...
binary_precedences = {}
for _ops, _val in zip(_binary_ptable, list(range(10, 20))):
...
class BinaryInfix(JavaExpression):
    ...
    @property
    def dtype(self):
        return self.params[0].dtype if self._dtype is None else self._dtype

    def addParens_sub(self):
        myprec = self.precedence
        associative = (
            myprec >= 15
        )  # for now we treat +, *, etc as nonassociative due to floats
    ...
    def tree(self, printer, tree):
        return [self.__class__.__name__, list(map(tree, self.params)), self.opstr]
...
class ClassInstanceCreation(JavaExpression):
    ...
    def print_(self, printer, print_):
        return "new {}({})".format(
            print_(self.typename), ", ".join(print_(x) for x in self.params)
        )

    def tree(self, printer, tree):
        return [
            self.__class__.__name__,
            list(map(tree, self.params)),
            tree(self.typename),
        ]
    ...
    def addCasts_sub(self, env):
        ...
        for tt, expr in zip(self.tts, self.params):
            if expr.dtype != tt and (
                ALWAYS_CAST_PARAMS or not isJavaAssignable(env, expr.dtype, tt)
            ):
...
def printFloat(x, isSingle):
    ...
    if isSingle and x > 0.0:
        ...
        if (
            lbound < float(s) < ubound
        ):  # strict ineq to avoid potential double rounding issues
            return s + suffix
...
class Literal(JavaExpression):
    ...
    def tree(self, printer, tree):
        result = (
            tree(self.params[0]) if self.dtype == objtypes.ClassTT else self.getStr()
        )
        return [self.__class__.__name__, result, self.dtype]

    def _key(self):
        return self.dtype, self.val
...
class Local(JavaExpression):
    ...
    def tree(self, printer, tree):
        return [self.__class__.__name__, self.print_(None, None)]
...
class MethodInvocation(JavaExpression):
    ...
    def print_(self, printer, print_):
        ...
        if self.hasLeft:
            ...
            return "{}.{}({})".format(
                print_(left), name, ", ".join(print_(x) for x in arguments)
            )
        else:
...
class TypeName(JavaExpression):
    ...
    def tree(self, printer, tree):
        return [self.__class__.__name__, self.tt]

    def complexity(self):
        return (
            -1
        )  # exprs which have this as a param won't be bumped up to 1 uncessarily


class CatchTypeNames(
    JavaExpression
):  # Used for caught exceptions, which can have multiple types specified
    def __init__(self, env, tts):
        assert tts and not any(
            objtypes.dim(tt) for tt in tts
        )  # at least one type, no array types
    ...
    def tree(self, printer, tree):
        return [self.__class__.__name__, list(map(tree, self.tnames))]
...
class UnaryPrefix(JavaExpression):
    ...
    def addParens_sub(self):
        p0 = self.params[0]
        if p0.precedence > 5 or (
            isinstance(p0, UnaryPrefix) and p0.opstr[0] == self.opstr[0]
        ):
            self.params[0] = Parenthesis(p0)

    def tree(self, printer, tree):
        return ["Unary", list(map(tree, self.params)), self.opstr, False]
