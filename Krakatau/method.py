...
exceptionHandlerRaw = collections.namedtuple(
    "exceptionHandlerRaw", ["start", "end", "handler", "type_ind"]
)
...
class Code(object):
    ...
    def __str__(self):  # pragma: no cover
        ...
        lines += [
            "{}: {}".format(i, bytecode.printInstruction(instructions[i]))
...
class Method(object):
    ...
    def __init__(self, data, classFile, keepRaw):
        ...
        self.flags = set(
            name for name, mask in list(Method.flagVals.items()) if (mask & flags)
        )
    ...
    def _checkFlags(self):
        ...
        if "ABSTRACT" in self.flags:
            assert not self.flags & set(
                ["SYNCHRONIZED", "PRIVATE", "FINAL", "STRICT", "STATIC", "NATIVE"]
            )
