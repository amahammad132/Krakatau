...
def _runJava(target, in_fname, argslist):
    ...
    if isclass:
        ...
        func = lambda args: execute(
            ["java", "-cp", target + ".jar", target] + list(args), cwd=tdir
        )
...
def _disassemble(contents, roundtrip):
    with script_util.MockWriter() as out:
        disassemble.disassembleSub(
            contents.get, out, list(contents), roundtrip=roundtrip
        )
...
if __name__ == "__main__":
    ...
    if do_decompile:
        for target, testcases in filter(
            target_filter, sorted(tests.decompiler.registry.items())
        ):
            testlist.append(("decompiler", target, list(map(tuple, testcases))))
    if do_disassemble:
        for target, testcases in filter(
            target_filter, sorted(tests.disassembler.registry.items())
        ):
            testlist.append(
                (
                    "disassembler",
        ...
        for fname in os.listdir(dis2_class_location):
            ...
            if target_filter((target, [])):
                testlist.append(
                    ("disassembler", True, dis2_class_location, target, None)
                )

    if do_assemble:
        test_base = os.path.join(krakatau_root, "tests")
        addAssemblerTests(
            testlist, target_filter, os.path.join(test_base, "assembler", "bad"), True
        )
        addAssemblerTests(
            testlist, target_filter, os.path.join(test_base, "assembler", "good"), False
        )
        addAssemblerTests(
            testlist,
            target_filter,
            os.path.join(test_base, "decompiler", "source"),
            False,
        )
        addAssemblerTests(
            testlist,
            target_filter,
            os.path.join(test_base, "disassembler", "source"),
            False,
        )
