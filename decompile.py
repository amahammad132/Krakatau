...
def findJRE():
    try:
        ...
        out = subprocess.Popen(
            ["update-java-alternatives", "-l"], stdout=subprocess.PIPE
        ).communicate()[0]
        basedir = out.split("\n")[0].rpartition(" ")[-1]
        ...
        for i, target in enumerate(targets):
            ...
            try:
                ...
                source = printer.visit(
                    javaclass.generateAST(
                        c, makeGraphCB, skip_errors, add_throws=add_throws
                    )
                )
                ...
                if isinstance(err, ClassLoaderError):
                    print(
                        "Failed to decompile {} due to missing or invalid class {}".format(
                            target, err.data
                        )
                    )
                else:
            ...
            if b"/" in target:
                ...
                package = "package {};\n\n".format(
                    (target.replace(b"/", b".").rpartition(b".")[0]).decode()
                )
...
def set_parser_opts(parser):
    parser.add_argument(
        "-path",
        action="append",
        help="Semicolon seperated paths or jars to search when loading classes",
    )
    ...
    parser.add_argument(
        "-skip",
        action="store_true",
        help="Upon errors, skip class or method and continue decompiling",
    )
